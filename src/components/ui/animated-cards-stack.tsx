...
const cardVariants = cva("absolute will-change-transform overflow-hidden", {
  variants: {
    variant: {
      dark: "flex size-full flex-col items-center justify-center gap-6 rounded-[2rem] border border-white/10 bg-[#09090b]/80 p-6 shadow-xl backdrop-blur-xl",
      light:
        "flex size-full flex-col items-center justify-center gap-6 rounded-[2rem] border border-black/10 bg-white/80 p-6 shadow-xl backdrop-blur-xl",
    },
  },
  defaultVariants: {
    variant: "light",
...
export const ContainerScroll: React.FC<
  React.HTMLAttributes<HTMLDivElement> & { containerId?: string }
> = ({ children, style, className, containerId, ...props }) => {
  ...
  if (!activeContainer) {
    return <div ref={scrollRef} className="opacity-0 w-full min-h-[500px]" />;
  }
...
const ContainerScrollInner: React.FC<
  React.HTMLAttributes<HTMLDivElement> & { scrollContainer: HTMLElement }
> = ({ children, style, className, scrollContainer, ...props }) => {
  const scrollRef = React.useRef<HTMLDivElement>(null);
